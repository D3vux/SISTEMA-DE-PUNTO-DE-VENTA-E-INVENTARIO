...
<div class="container">
    ...
    $result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM product WHERE product_name LIKE '%$name%'");
    $row = mysqli_fetch_assoc($result);
    $total_productos = $row['total'];
    $total_paginas = ceil($total_productos / $productos_por_pagina);

    // Obtener los productos de la página actual que coinciden con la búsqueda
    $query = mysqli_query($conn, "SELECT * FROM product WHERE product_name LIKE '%$name%' LIMIT $offset, $productos_por_pagina");
