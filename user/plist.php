...
<div class="container">
    ...
    $productos_por_pagina = 8; // Puedes ajustar este valor según tus necesidades

    // Obtener el número de la página actual
    $pagina = isset($_GET['pagina']) ? (int)$_GET['pagina'] : 1;
    $offset = ($pagina - 1) * $productos_por_pagina;

    // Contar el total de productos de la categoría
    $result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM product WHERE categoryid='$cat'");
    $row = mysqli_fetch_assoc($result);
    $total_productos = $row['total'];
    $total_paginas = ceil($total_productos / $productos_por_pagina);

    // Obtener los productos de la página actual
    $query = mysqli_query($conn, "SELECT * FROM product WHERE categoryid='$cat' LIMIT $offset, $productos_por_pagina");
    ...
    for ($i = 1; $i <= $total_paginas; $i++) {
        echo "<li class='page-item'><a class='page-link' href='?cat=$cat&pagina=$i'>$i</a></li>";
    }
    echo "</ul></nav>";
